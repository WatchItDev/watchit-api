...
export class LogService extends ServiceManager {
  logEvent = (addr: string, payload: any) => this.ds.Logs.logEvent(addr, { ...payload });
  ...
  countPostViews = (postId: string) => this.ds.Logs.countEvents('POST_VIEW', 'targetId', postId);

  countProfileViews = (addr: string) => this.ds.Logs.countEvents('PROFILE_VIEW', 'targetId', addr);
}
