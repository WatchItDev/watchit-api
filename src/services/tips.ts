...
export class TipsService extends ServiceManager {
    async createTip(baker: string, input: {
        postId: string;
        creator: string;
        amount: number;
        txHash?: string | null;
    ...
    async getTipsByBakerForPost(postId: string, limit = 50): Promise<
        Array<{ baker: User; totalAmount: number; count: number; lastTipAt: number; }>
    > {
