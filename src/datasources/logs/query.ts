...
export class LogsQuery extends DataSourceManager {
    ...
    eventsByTarget(id: string,
                   tgtType?: string,
                   type?: string,
                   limit = 50,
                   offset = 0) {
        const clauses: Clause[] = [
            { field: 'targetId', op: '==', value: id },
        ]
        if (tgtType) clauses.push({ field: 'targetType', op: '==', value: tgtType })
        if (type)    clauses.push({ field: 'type',       op: '==', value: type })
    ...
    countEvents(type: string, idField: 'targetId' | 'author', id: string) {
        ...
        return this.fs('eventLogs').query(clauses).then(r => r.length)
    }
}
