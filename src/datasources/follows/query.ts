...
export class FollowsQuery extends DataSourceManager {
    ...
    async followersOf(address: string, limit = 50): Promise<User[]> {
        const rows = await this.fs<{ follower: string }>('follows')
            .query([{ field: 'following', op: '==', value: address }], { limit })
    ...
    async followingOf(address: string, limit = 50): Promise<User[]> {
        const rows = await this.fs<{ following: string }>('follows')
            .query([{ field: 'follower', op: '==', value: address }], { limit })
        if (!rows.length) return []
